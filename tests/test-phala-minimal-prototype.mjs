...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "phala-minimal-prototype.json");
...
async function main() {
  ...
  try {
    ...
    const sampleTelemetry = {
      nodeAlias: "phala-minimal-prototype-node",
      channels: [
        {
          channelId: "100x1x0",
          peerPubkey: "02aaaa",
    ...
    assert(healthResponse.ok === true, "Phala Minimal Prototype failed: /health did not return ok=true.");
    assert(snapshotResponse.status === 200, "Phala Minimal Prototype failed: /api/snapshot did not return 200.");
    assert(snapshotResponse.body.ok === true, "Phala Minimal Prototype failed: /api/snapshot did not return ok=true.");
    assert(recommendResponse.status === 200, "Phala Minimal Prototype failed: /api/recommend did not return 200.");
    assert(
      recommendResponse.body.recommendationSet?.feeRecommendations?.length === 1,
      "Phala Minimal Prototype failed: /api/recommend did not return deterministic fee recommendations."
    );
    assert(verifyPassResponse.body.ok === true, "Phala Minimal Prototype failed: prototype verify pass response should be ok=true.");
    assert(verifyFailResponse.body.ok === false, "Phala Minimal Prototype failed: prototype verify fail response should be ok=false.");
    ...
    assert(dockerfileSource.includes("server.mjs"), "Phala Minimal Prototype failed: Dockerfile no longer documents the standalone server image.");
    assert(composeSource.includes("8787:8787"), "Phala Minimal Prototype failed: docker-compose is missing port mapping.");
    assert(composeSource.includes("image: node:22-alpine"), "Phala Minimal Prototype failed: docker-compose is not using the stock Node alpine image.");
    assert(composeSource.includes("platform: linux/amd64"), "Phala Minimal Prototype failed: docker-compose is missing explicit linux/amd64 platform.");
    assert(composeSource.includes("- -e"), "Phala Minimal Prototype failed: docker-compose is not launching node with -e.");
    assert(deployScriptSource.includes("phala deploy"), "Phala Minimal Prototype failed: deploy script does not call phala deploy.");

    const artifact = {
      schemaVersion: "phala-minimal-prototype-v1",
      healthResponse,
    ...
    console.log(`Saved Phala Minimal Prototype artifact: ${ARTIFACT_PATH}`);
    ...
    console.log("Phala Minimal Prototype test: PASS");
  } finally {
    child.kill("SIGTERM");
  }
}
...
main().catch((error) => {
  console.error("Phala Minimal Prototype test failed.", error);
  process.exitCode = 1;
});
