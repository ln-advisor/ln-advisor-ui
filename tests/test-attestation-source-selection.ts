...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "attestation-source-selection.json");
...
async function main(): Promise<void> {
  ...
  assert(cloudSourceResult.ok, "Attestation Source Selection failed: cloud source verification should pass.");
  assert(cloudSourceResult.checks.composeHashMatchesRtmr3Event === null, "Attestation Source Selection failed: cloud compose check should be null.");
  assert(appSourceResult.ok, "Attestation Source Selection failed: app source verification should pass.");
  assert(
    appSourceResult.checks.composeHashMatchesRtmr3Event === true,
    "Attestation Source Selection failed: app source compose check should pass."
  );
  assert(!appSourceMismatchResult.ok, "Attestation Source Selection failed: mismatched report data should fail app source.");
  ...
  for (const call of cloudCalls) {
    assert(
      ...
      `Attestation Source Selection failed: missing ${PHALA_API_VERSION_HEADER} on ${call.method} ${call.path}.`
    );
  }

  const artifact = {
    schemaVersion: "attestation-source-selection-v1",
    sources: {
  ...
  console.log(`Saved Attestation Source Selection artifact: ${ARTIFACT_PATH}`);
  ...
  console.log("Attestation Source Selection test: PASS");
}

main().catch((error) => {
  console.error("Attestation Source Selection test failed.", error);
  process.exitCode = 1;
});
