...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "tee-simulated.json");
const FIXED_DEV_SIGNING_KEY = "tee-simulated-dev-signing-key";
...
async function main(): Promise<void> {
  ...
  assert(verifyResult.ok, `Tee Simulated verification failed: ${verifyResult.errors.join(" | ")}`);

  assert(firstRun.arb.attestation, "Tee Simulated failed: ARB is missing attestation evidence.");
  assert(
    firstRun.arb.attestation?.executionMode === "tee_simulated",
    "Tee Simulated failed: attestation execution mode is not tee_simulated."
  );
  assert(
    firstRun.sourceProvenance.executionContext.executionMode === "tee_candidate",
    "Tee Simulated failed: provenance execution mode should map tee_simulated -> tee_candidate."
  );
  assert(
    firstRun.sourceProvenance.executionContext.enclaveProviderId === simulatedTeeEnclaveProvider.providerId,
    "Tee Simulated failed: provenance provider does not match simulated TEE provider."
  );
  assert(
    firstRun.runSummary.attestation.providerId === simulatedTeeEnclaveProvider.providerId,
    "Tee Simulated failed: run summary provider mismatch."
  );

  const deterministic = canonicalJson(firstRun.arb) === canonicalJson(secondRun.arb);
  assert(deterministic, "Tee Simulated failed: ARB output is not deterministic for identical inputs.");

  const artifact = {
    schemaVersion: "tee-simulated-v1",
    issuedAt: FIXED_ISSUED_AT,
    provider: {
  ...
  console.log(`Saved Tee Simulated artifact: ${ARTIFACT_PATH}`);
  console.log(`TEE provider: ${firstRun.arb.attestation?.providerId}`);
  console.log("Tee Simulated test: PASS");
}

main().catch((error) => {
  console.error("Tee Simulated test failed.", error);
  process.exitCode = 1;
});
