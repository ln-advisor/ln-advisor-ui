...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "phala-minimal-real-attestation-source.json");
...
const CLOUD_VERSION = "2026-03-14-phala-minimal-real-attestation-source";
const CLOUD_API_KEY = "phala-minimal-real-attestation-source-cloud-api-key";
...
const sampleTelemetry = {
  nodeAlias: "phala-phala-minimal-real-attestation-source-node",
  channels: [
    {
      channelId: "100x1x0",
      peerPubkey: "02aaaa",
...
async function runSyncWithRenderValues() {
  ...
  assert(exitCode === 0, "Phala Minimal Real Attestation Source failed: sync script did not exit cleanly.");
  return {
    stdout: stdout.trim(),
    stderr: stderr.trim(),
  };
...
async function startPrototypeServer(port, extraEnv) {
  const child = spawn(process.execPath, [SERVER_PATH], {
    ...
    env: {
      ...
      PROTOTYPE_SERVICE_NAME: `ln-advisor-phala-minimal-phala-minimal-real-attestation-source-${port}`,
      PROTOTYPE_ARB_SIGNING_KEY: "phala-minimal-real-attestation-source-signing-key",
      PROTOTYPE_SIGNER_PROVIDER_ID: "prototype-env-signer-phala-minimal-real-attestation-source",
      ...
      PROTOTYPE_ATTESTATION_MEASUREMENT: "phala-minimal-real-attestation-source-measurement",
      PROTOTYPE_ATTESTATION_SOURCE: "dstack_runtime",
      PROTOTYPE_VERIFY_ALLOWED_ATTESTATION_PROVIDER_ID: "phala-cloud",
      PROTOTYPE_VERIFY_ALLOWED_MEASUREMENT: "phala-minimal-real-attestation-source-measurement",
...
async function main() {
  ...
  const fakeDstackServer = http.createServer(async (req, res) => {
    if (req.url === "/Info") {
      ...
      const payload = {
        app_id: "phala-minimal-real-attestation-source-app-id",
        instance_id: "phala-minimal-real-attestation-source-instance-id",
        tcb_info: {
      ...
      res.end(
        `${JSON.stringify({
          ...
          quote: {
            ...
            hex: body?.hex || null,
            body: {
              mrtd: "phala-minimal-real-attestation-source-measurement",
            },
          },
        })}\n`
      );
  ...
  try {
    ...
    assert(recommend.status === 200, "Phala Minimal Real Attestation Source failed: runtime-backed recommend should return 200.");
    assert(
      recommend.body.arb?.attestation?.quote === RUNTIME_RECOMMEND_QUOTE_HEX,
      "Phala Minimal Real Attestation Source failed: ARB attestation quote should come from the first dstack runtime quote."
    );
    assert(info.body?.tcb_info?.app_compose === runtimeAppCompose, "Phala Minimal Real Attestation Source failed: /info should return runtime app_compose.");
    assert(
      attestation.body?.quote === RUNTIME_LIVE_ATTESTATION_QUOTE_HEX,
      "Phala Minimal Real Attestation Source failed: /attestation should return the later runtime quote."
    );
    assert(attestation.body?.report_data === recommend.body.arb.digest, "Phala Minimal Real Attestation Source failed: /attestation report_data should match digest.");
    assert(verify.body.ok === true, "Phala Minimal Real Attestation Source failed: runtime-backed quote should pass verification.");
    assert(verify.body.cloudVerification?.quoteVerified === true, "Phala Minimal Real Attestation Source failed: cloud verification should accept runtime-backed quote.");
    assert(
      ...
      "Phala Minimal Real Attestation Source failed: runtime compose-hash event-log mismatch should downgrade to warning."
    );
    assert(brokenRecommend.status === 503, "Phala Minimal Real Attestation Source failed: required dstack runtime source should fail closed when unavailable.");

    assert(dstackCalls.some((call) => call.path === "/Info"), "Phala Minimal Real Attestation Source failed: fake dstack Info endpoint was not called.");
    assert(dstackCalls.some((call) => call.path === "/GetQuote"), "Phala Minimal Real Attestation Source failed: fake dstack GetQuote endpoint was not called.");
    assert(cloudCalls.length >= 1, "Phala Minimal Real Attestation Source failed: fake cloud verify endpoint was not called.");
    assert(cloudCalls[0].headers["x-api-key"] === CLOUD_API_KEY, "Phala Minimal Real Attestation Source failed: missing X-API-Key header.");
    assert(cloudCalls[0].headers["x-phala-version"] === CLOUD_VERSION, "Phala Minimal Real Attestation Source failed: missing X-Phala-Version header.");
    assert(
      cloudCalls.some((call) => call.body?.hex === RUNTIME_RECOMMEND_QUOTE_HEX),
      "Phala Minimal Real Attestation Source failed: cloud verification should receive the runtime quote used for ARB generation."
    );
    assert(
      cloudCalls.some((call) => call.body?.hex === RUNTIME_LIVE_ATTESTATION_QUOTE_HEX),
      "Phala Minimal Real Attestation Source failed: cloud verification should receive the live attestation quote hex."
    );
    ...
    assert(templateCompose.includes("/var/run/dstack.sock:/var/run/dstack.sock"), "Phala Minimal Real Attestation Source failed: compose scaffold missing dstack.sock mount.");
    assert(templateCompose.includes('${PROTOTYPE_ATTESTATION_SOURCE:-prototype_env}'), "Phala Minimal Real Attestation Source failed: template compose missing attestation source placeholder.");
    assert(templateCompose.includes('${PROTOTYPE_DSTACK_ENDPOINT:-}'), "Phala Minimal Real Attestation Source failed: template compose missing dstack endpoint placeholder.");
    assert(renderedCompose.includes('PROTOTYPE_ATTESTATION_SOURCE: "dstack_runtime"'), "Phala Minimal Real Attestation Source failed: rendered compose missing concrete attestation source.");
    assert(renderedCompose.includes(`PROTOTYPE_DSTACK_ENDPOINT: "http://127.0.0.1:${DSTACK_PORT}"`), "Phala Minimal Real Attestation Source failed: rendered compose missing concrete dstack endpoint.");

    const artifact = {
      schemaVersion: "phala-minimal-real-attestation-source-v1",
      good: {
    ...
    console.log(`Saved Phala Minimal Real Attestation Source artifact: ${ARTIFACT_PATH}`);
    console.log(
    ...
    console.log("Phala Minimal Real Attestation Source test: PASS");
  } finally {
...
main().catch((error) => {
  console.error("Phala Minimal Real Attestation Source test failed.", error);
  process.exitCode = 1;
});
