...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "tee-verified-policy.json");
const FIXED_DEV_SIGNING_KEY = "tee-verified-policy-dev-signing-key";
...
async function main(): Promise<void> {
  ...
  assert(verifiedVerifyResult.ok, `Tee Verified Policy verifyArb failed: ${verifiedVerifyResult.errors.join(" | ")}`);
  ...
  assert(
    ...
    `Tee Verified Policy policy failed for verified run: ${verifiedPolicyResult.errors.join(" | ")}`
  );
  ...
  assert(!simulatedPolicyResult.ok, "Tee Verified Policy expected tee_simulated run to fail tee_verified policy.");

  const artifact = {
    schemaVersion: "tee-verified-policy-v1",
    policy,
    verifiedRun: {
  ...
  console.log(`Saved Tee Verified Policy artifact: ${ARTIFACT_PATH}`);
  ...
  console.log("Tee Verified Policy test: PASS");
}

main().catch((error) => {
  console.error("Tee Verified Policy test failed.", error);
  process.exitCode = 1;
});
