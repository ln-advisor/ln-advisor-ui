...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "private-model-pinning.json");
...
const FIXED_SIGNING_KEY = "private-model-pinning-dev-signing-key";
...
async function main(): Promise<void> {
  ...
  assert(verifyResult.ok, `Private Model Pinning failed: private-model ARB did not verify (${verifyResult.errors.join(" | ")})`);
  assert(
    arb.modelPinningMode === "service_pinned_private_model",
    "Private Model Pinning failed: ARB did not preserve service_pinned_private_model pinning mode."
  );
  ...
  assert(Object.values(filesExist).every(Boolean), "Private Model Pinning failed: Phala deploy scaffold files are missing.");

  const composeSource = await readFile(COMPOSE_PATH, "utf8");
  const deployScriptSource = await readFile(DEPLOY_SCRIPT_PATH, "utf8");
  assert(composeSource.includes("/var/run/dstack.sock"), "Private Model Pinning failed: compose file is missing dstack.sock mount.");
  assert(composeSource.includes("8787:8787"), "Private Model Pinning failed: compose file is missing API port mapping.");
  assert(deployScriptSource.includes("phala deploy"), "Private Model Pinning failed: deploy script does not call phala deploy.");

  const artifact = {
    schemaVersion: "private-model-pinning-v1",
    privateManifest,
  ...
  console.log(`Saved Private Model Pinning artifact: ${ARTIFACT_PATH}`);
  ...
  console.log("Private Model Pinning test: PASS");
}

main().catch((error) => {
  console.error("Private Model Pinning test failed.", error);
  process.exitCode = 1;
});
