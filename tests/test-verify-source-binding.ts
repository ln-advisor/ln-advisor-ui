...
import { verifySourceVerificationBinding } from "../src/arb/verifySourceVerificationBinding";

const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "verify-source-binding.json");
...
const FIXED_DEV_SIGNING_KEY = "verify-source-binding-dev-signing-key";
...
async function main(): Promise<void> {
  ...
  const verifyArbResult = verifyArb({
    arb: run.arb,
    sourceProvenance: run.sourceProvenance,
    devSigningKey: FIXED_DEV_SIGNING_KEY,
    now: FIXED_VERIFY_NOW,
  });
  assert(verifyArbResult.ok, `Verify Source Binding verifyArb failed: ${verifyArbResult.errors.join(" | ")}`);

  const bindingPass = verifySourceVerificationBinding({
    sourceProvenance: run.sourceProvenance,
    sourceVerificationResult: cloudSourceVerification,
    requireSourceVerification: true,
  });
  assert(bindingPass.ok, `Verify Source Binding binding should pass: ${bindingPass.errors.join(" | ")}`);

  const bindingWrongSource = verifySourceVerificationBinding({
    sourceProvenance: run.sourceProvenance,
    sourceVerificationResult: appSourceVerification,
    requireSourceVerification: true,
  });
  assert(!bindingWrongSource.ok, "Verify Source Binding failed: mismatched source verification should be rejected.");

  const bindingMissing = verifySourceVerificationBinding({
    sourceProvenance: run.sourceProvenance,
    requireSourceVerification: true,
  });
  assert(!bindingMissing.ok, "Verify Source Binding failed: missing source verification should be rejected.");
  ...
  for (const call of cloudCalls) {
    assert(
      ...
      `Verify Source Binding failed: missing ${PHALA_API_VERSION_HEADER} header on ${call.method} ${call.path}.`
    );
  }

  const artifact = {
    schemaVersion: "verify-source-binding-v1",
    inputs: {
      cvmInfoPath,
      cliAttestationPath,
    },
    verifyArb: verifyArbResult,
    sourceVerificationBinding: {
      pass: bindingPass,
      wrongSource: bindingWrongSource,
      missing: bindingMissing,
    },
    doneCondition:
      "Verifier-side source binding is enforceable: provenance must match the exact source verification receipt; mismatched or missing receipts are rejected.",
  };
  ...
  console.log(`Saved Verify Source Binding artifact: ${ARTIFACT_PATH}`);
  console.log(`Binding pass: ${bindingPass.ok}`);
  console.log(`Wrong source rejected: ${!bindingWrongSource.ok}`);
  console.log(`Missing source rejected: ${!bindingMissing.ok}`);
  console.log("Verify Source Binding test: PASS");
}

main().catch((error) => {
  console.error("Verify Source Binding test failed.", error);
  process.exitCode = 1;
});
