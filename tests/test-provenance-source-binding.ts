...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "provenance-source-binding.json");
...
const FIXED_DEV_SIGNING_KEY = "provenance-source-binding-dev-signing-key";
...
async function main(): Promise<void> {
  ...
  assert(cloudVerify.ok, `Provenance Source Binding cloud ARB verify failed: ${cloudVerify.errors.join(" | ")}`);
  assert(appVerify.ok, `Provenance Source Binding app ARB verify failed: ${appVerify.errors.join(" | ")}`);
  ...
  assert(cloudCtx.sourceVerificationSource === "cloud_cvm_attestation", "Provenance Source Binding failed: cloud source label mismatch.");
  assert(appCtx.sourceVerificationSource === "app_http_attestation", "Provenance Source Binding failed: app source label mismatch.");
  assert(cloudCtx.sourceVerificationHash === expectedCloudSourceHash, "Provenance Source Binding failed: cloud source hash mismatch.");
  assert(appCtx.sourceVerificationHash === expectedAppSourceHash, "Provenance Source Binding failed: app source hash mismatch.");
  ...
  assert(
    ...
    "Provenance Source Binding failed: source verification hash should differ between cloud and app source receipts."
  );
  assert(
    cloudRun.arb.sourceProvenanceHash !== appRun.arb.sourceProvenanceHash,
    "Provenance Source Binding failed: sourceProvenanceHash should change when source verification receipt changes."
  );

  const deterministic = canonicalJson(cloudRun.arb) === canonicalJson(cloudRunAgain.arb);
  assert(deterministic, "Provenance Source Binding failed: cloud-bound ARB output is not deterministic for fixed inputs.");
  ...
  for (const call of cloudCalls) {
    assert(
      ...
      `Provenance Source Binding failed: missing ${PHALA_API_VERSION_HEADER} header on ${call.method} ${call.path}.`
    );
  }

  const artifact = {
    schemaVersion: "provenance-source-binding-v1",
    inputs: {
      cvmInfoPath,
      cliAttestationPath,
    },
  ...
  console.log(`Saved Provenance Source Binding artifact: ${ARTIFACT_PATH}`);
  ...
  console.log("Provenance Source Binding test: PASS");
}

main().catch((error) => {
  console.error("Provenance Source Binding test failed.", error);
  process.exitCode = 1;
});
