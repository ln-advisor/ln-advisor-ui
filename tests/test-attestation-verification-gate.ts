...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "attestation-verification-gate.json");
...
async function main(): Promise<void> {
  ...
  assert(cloudGateResult.ok, `Attestation Verification Gate failed: cloud source gate should pass (${cloudGateResult.errors.join(" | ")})`);
  ...
  assert(appGateResult.ok, `Attestation Verification Gate failed: app source gate should pass (${appGateResult.errors.join(" | ")})`);
  ...
  assert(!appMismatchGateResult.ok, "Attestation Verification Gate failed: reportData mismatch should fail verification gate.");
  ...
  assert(!sourceRestrictedGateResult.ok, "Attestation Verification Gate failed: source restriction should reject app source.");
  ...
  assert(!quoteFormatMismatchGateResult.ok, "Attestation Verification Gate failed: quote format mismatch should be rejected.");
  ...
  for (const call of cloudCalls) {
    assert(
      ...
      `Attestation Verification Gate failed: missing ${PHALA_API_VERSION_HEADER} header on ${call.method} ${call.path}.`
    );
  }

  const artifact = {
    schemaVersion: "attestation-verification-gate-v1",
    policy: strictGatePolicy,
    sourceVerification: {
  ...
  console.log(`Saved Attestation Verification Gate artifact: ${ARTIFACT_PATH}`);
  ...
  console.log("Attestation Verification Gate test: PASS");
}

main().catch((error) => {
  console.error("Attestation Verification Gate test failed.", error);
  process.exitCode = 1;
});
