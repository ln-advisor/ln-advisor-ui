...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "strict-attestation-policy.json");
...
const FIXED_DEV_SIGNING_KEY = "strict-attestation-policy-dev-signing-key";
...
async function main(): Promise<void> {
  ...
  assert(verifyResult.ok, `Strict Attestation Policy verifyArb failed: ${verifyResult.errors.join(" | ")}`);

  const attestation = run.arb.attestation;
  assert(attestation, "Strict Attestation Policy failed: ARB is missing attestation evidence.");
  ...
  assert(
    ...
    `Strict Attestation Policy strict policy should pass but failed: ${strictPolicyResult.errors.join(" | ")}`
  );
  ...
  assert(
    ...
    "Strict Attestation Policy failed: wrong-measurement policy should reject this ARB."
  );
  ...
  assert(!wrongQuotePolicyResult.ok, "Strict Attestation Policy failed: wrong-quote-format policy should reject this ARB.");
  ...
  assert(!wrongProviderPolicyResult.ok, "Strict Attestation Policy failed: wrong-provider policy should reject this ARB.");

  const artifact = {
    schemaVersion: "strict-attestation-policy-v1",
    inputs: {
      cvmInfoPath,
      cliAttestationPath,
    },
  ...
  console.log(`Saved Strict Attestation Policy artifact: ${ARTIFACT_PATH}`);
  ...
  console.log("Strict Attestation Policy test: PASS");
}

main().catch((error) => {
  console.error("Strict Attestation Policy test failed.", error);
  process.exitCode = 1;
});
