...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "attestation-quote-formats.json");
...
const FIXED_DEV_SIGNING_KEY = "attestation-quote-formats-dev-signing-key";
...
async function main(): Promise<void> {
  ...
  assert(verifyResult.ok, `Attestation Quote Formats verifyArb failed: ${verifyResult.errors.join(" | ")}`);
  assert(tdxRun.arb.attestation, "Attestation Quote Formats failed: ARB is missing attestation evidence.");
  assert(
    tdxRun.arb.attestation?.quoteFormat === "tdx_quote",
    `Attestation Quote Formats failed: expected tdx_quote but got ${tdxRun.arb.attestation?.quoteFormat || "null"}.`
  );
  assert(tdxRun.runSummary.keyRelease.granted, "Attestation Quote Formats failed: key release should be granted for tdx_quote policy.");
  ...
  assert(deniedMessage.includes("Key release denied:"), "Attestation Quote Formats failed: expected quote format policy denial.");
  ...
  assert(deterministic, "Attestation Quote Formats failed: ARB output is not deterministic for fixed inputs.");

  const artifact = {
    schemaVersion: "attestation-quote-formats-v1",
    inputs: {
      cvmInfoPath,
      cliAttestationPath,
    },
  ...
  console.log(`Saved Attestation Quote Formats artifact: ${ARTIFACT_PATH}`);
  ...
  console.log("Attestation Quote Formats test: PASS");
}

main().catch((error) => {
  console.error("Attestation Quote Formats test failed.", error);
  process.exitCode = 1;
});
