...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "retention-policy.json");
...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert(noneResponse.status === 200, "Retention Policy failed: retention none request did not return 200.");
    assert(noneResponse.body.retention?.retained === false, "Retention Policy failed: default retention should not persist data.");
    ...
    assert(featureResponse.status === 200, "Retention Policy failed: feature_only retention request did not return 200.");
    const featurePath = String(featureResponse.body.retention?.artifactPath || "");
    assert(featureResponse.body.retention?.retained === true, "Retention Policy failed: feature-only retention should persist data.");
    assert(await fileExists(featurePath), "Retention Policy failed: feature-only retention artifact is missing.");
    const featurePayload = JSON.parse(await readFile(featurePath, "utf8")) as Record<string, unknown>;
    assert(featurePayload.privacyMode === "feature_only", "Retention Policy failed: retained feature-only payload has wrong privacyMode.");
    ...
    assert(bandedResponse.status === 200, "Retention Policy failed: banded retention request did not return 200.");
    const bandedPath = String(bandedResponse.body.retention?.artifactPath || "");
    assert(bandedResponse.body.retention?.retained === true, "Retention Policy failed: banded retention should persist data.");
    assert(await fileExists(bandedPath), "Retention Policy failed: banded retention artifact is missing.");
    const bandedRaw = await readFile(bandedPath, "utf8");
    const bandedPayload = JSON.parse(bandedRaw) as Record<string, unknown>;
    assert(bandedPayload.privacyMode === "banded", "Retention Policy failed: retained banded payload has wrong privacyMode.");
    assert(!bandedRaw.includes("\"channelId\""), "Retention Policy failed: banded retention leaked exact channelId.");
    assert(!bandedRaw.includes("\"localBalanceSat\""), "Retention Policy failed: banded retention leaked exact localBalanceSat.");
    assert(!bandedRaw.includes("\"remoteBalanceSat\""), "Retention Policy failed: banded retention leaked exact remoteBalanceSat.");
    ...
    console.log(`Saved Retention Policy artifact: ${ARTIFACT_PATH}`);
    ...
    console.log("Retention Policy test: PASS");
  } finally {
    server.close();
  }
}
...
main().catch((error) => {
  console.error("Retention Policy test failed.", error);
  process.exitCode = 1;
});
