...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "source-gated-recommend.json");
...
const RELEASED_KEY_ID = "arb-signer-source-gated-recommend-v1";
const RELEASED_SIGNING_KEY = "source-gated-recommend-released-signing-key-material";
const DECOY_DEV_KEY = "source-gated-recommend-decoy-dev-key";
const KEY_PROVIDER_ID = "source-gated-recommend-v1";
...
const mockSourceVerification: VerifyPhalaAttestationBySourceResult = {
  ...
  verifier: {
    ...
    note: "source-gated-recommend mock source verification",
  },
};
...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert(
      ...
      "Source Gated Recommend failed: strict recommend should fail closed when source verification runtime is not configured."
    );
    ...
    try {
      ...
      assert(firstPassResponse.status === 200, "Source Gated Recommend failed: strict recommend did not return 200.");
      assert(secondPassResponse.status === 200, "Source Gated Recommend failed: second strict recommend did not return 200.");
      assert(firstPassResponse.body.ok === true, "Source Gated Recommend failed: recommend response ok flag is false.");
      assert(
        firstPassResponse.body.signingMode === "released_signer",
        "Source Gated Recommend failed: signingMode is not released_signer."
      );
      ...
      assert(Boolean(sourceGate), "Source Gated Recommend failed: missing enclaveRunSummary.sourceVerificationGate.");
      assert(sourceGate?.policyApplied === true, "Source Gated Recommend failed: source verification gate should be policy-applied.");
      assert(sourceGate?.verified === true, "Source Gated Recommend failed: source verification gate should be verified.");
      assert(
        sourceGate?.source === mockSourceVerification.source,
        "Source Gated Recommend failed: source verification gate source mismatch."
      );
      assert(keyRelease?.releasedSignerUsed === true, "Source Gated Recommend failed: released signer should still be used.");
      ...
      assert(
        verifyReleased.ok,
        `Source Gated Recommend failed: ARB does not verify with released signing key (${verifyReleased.errors.join(" | ")})`
      );

      const deterministic = canonicalJson(firstPassResponse.body.arb) === canonicalJson(secondPassResponse.body.arb);
      assert(deterministic, "Source Gated Recommend failed: strict source-gated recommend output is not deterministic.");

      const artifact = {
        schemaVersion: "source-gated-recommend-v1",
        missingSourceRun: {
      ...
      console.log(`Saved Source Gated Recommend artifact: ${ARTIFACT_PATH}`);
      ...
      console.log("Source Gated Recommend test: PASS");
    } finally {
      passServer.close();
    }
  } finally {
...
main().catch((error) => {
  console.error("Source Gated Recommend test failed.", error);
  process.exitCode = 1;
});
