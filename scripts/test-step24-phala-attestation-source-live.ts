...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "step24.phala-attestation-source-live.json");
...
async function main(): Promise<void> {
  ...
  const artifact = {
    schemaVersion: "step24-phala-attestation-source-live-v1",
    config: {
      apiBaseUrl,
      apiVersion,
  ...
  console.log(`Saved Step 24 artifact: ${ARTIFACT_PATH}`);
  ...
  console.log(`Step 24 attestation source live test: ${overallOk ? "PASS" : "FAIL"}`);
...
main().catch((error) => {
  console.error("Step 24 attestation source live test failed.", error);
  process.exitCode = 1;
});
