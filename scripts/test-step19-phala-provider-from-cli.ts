...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "step19.phala-provider-from-cli.json");
...
const FIXED_DEV_SIGNING_KEY = "step19-dev-signing-key";
...
async function main(): Promise<void> {
  ...
  assert(verifyResult.ok, `Step19 verifyArb failed: ${verifyResult.errors.join(" | ")}`);
  assert(firstRun.arb.attestation, "Step19 failed: ARB is missing attestation evidence.");
  assert(
    firstRun.arb.attestation?.providerId === provider.providerId,
    "Step19 failed: ARB attestation providerId does not match Phala CLI provider."
  );
  assert(
    firstRun.arb.attestation?.executionMode === "tee_verified",
    "Step19 failed: ARB attestation execution mode is not tee_verified."
  );
  assert(
    firstRun.sourceProvenance.executionContext.executionMode === "tee_verified",
    "Step19 failed: provenance execution mode must be tee_verified."
  );
  assert(
    firstRun.sourceProvenance.executionContext.enclaveProviderId === provider.providerId,
    "Step19 failed: provenance enclaveProviderId does not match provider."
  );

  const deterministic = canonicalJson(firstRun.arb) === canonicalJson(secondRun.arb);
  assert(deterministic, "Step19 failed: ARB output is not deterministic for fixed inputs.");

  const artifact = {
    schemaVersion: "step19-phala-provider-from-cli-v1",
    inputs: {
      cvmInfoPath,
      cliAttestationPath,
    },
  ...
  console.log(`Saved Step 19 artifact: ${ARTIFACT_PATH}`);
  console.log(`Provider: ${provider.providerId}`);
  console.log("Step 19 phala provider-from-cli test: PASS");
}

main().catch((error) => {
  console.error("Step 19 phala provider-from-cli test failed.", error);
  process.exitCode = 1;
});
