...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "step33.trusted-verify.json");
const PORT = 8794;
const FIXED_ISSUED_AT = "2026-01-01T00:00:00.000Z";
const FIXED_VERIFY_NOW = "2026-01-01T12:00:00.000Z";
const RELEASED_KEY_ID = "arb-signer-step33-v1";
const RELEASED_SIGNING_KEY = "step33-released-signing-key-material";
const DECOY_DEV_KEY = "step33-decoy-dev-key";
const KEY_PROVIDER_ID = "step33-signing-provider-v1";
...
const mockSourceVerification: VerifyPhalaAttestationBySourceResult = {
  ...
  verifier: {
    ...
    note: "step33 mock source verification",
  },
};
...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert(recommendResponse.status === 200, "Step33 failed: recommend request did not return 200.");
    ...
    assert(devVerifyResponse.status === 200, "Step33 failed: dev verify request did not return 200.");
    assert(devVerifyResponse.body.ok === true, "Step33 failed: base verify should pass.");
    ...
    assert(strictVerifyResponse.status === 200, "Step33 failed: strict verify request did not return 200.");
    assert(strictVerifyResponse.body.ok === true, "Step33 failed: strict verify should pass.");
    ...
    assert(strictArbCheck?.ok === true, "Step33 failed: arb check should pass.");
    assert(strictAttestationCheck?.applied === true, "Step33 failed: attestation policy should be applied.");
    assert(strictSourceCheck?.applied === true, "Step33 failed: source binding should be applied.");
    ...
    assert(missingSourceVerifyResponse.status === 200, "Step33 failed: missing-source verify request did not return 200.");
    assert(missingSourceVerifyResponse.body.ok === false, "Step33 failed: strict verify should fail without source verification.");
    ...
    assert(
      ...
      "Step33 failed: arb check should remain valid even when trust policy fails."
    );
    ...
    assert(
      ...
      "Step33 failed: mismatched-source verify request did not return 200."
    );
    assert(
      mismatchedSourceVerifyResponse.body.ok === false,
      "Step33 failed: strict verify should fail when source binding mismatches."
    );

    const artifact = {
      schemaVersion: "step33-trusted-verify-v1",
      passRun: {
    ...
    console.log(`Saved Step 33 artifact: ${ARTIFACT_PATH}`);
    ...
    console.log("Step 33 trust-aware verify test: PASS");
  } finally {
    server.close();
...
main().catch((error) => {
  console.error("Step 33 trust-aware verify test failed.", error);
  process.exitCode = 1;
});
