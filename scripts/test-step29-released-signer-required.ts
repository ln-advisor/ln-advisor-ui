...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "step29.released-signer-required.json");
...
const DECOY_DEV_KEY = "step29-decoy-dev-signing-key-not-used";
...
async function main(): Promise<void> {
  ...
  const releasedKeyMaterial = "step29-released-signing-key-material";
  ...
  assert(verifyResult.ok, `Step29 verifyArb failed: ${verifyResult.errors.join(" | ")}`);

  assert(firstRun.runSummary.keyRelease.releasedSignerRequired, "Step29 failed: release-required flag not set.");
  assert(firstRun.runSummary.keyRelease.releasedSignerUsed, "Step29 failed: released signer was not used.");
  assert(
    firstRun.runSummary.keyRelease.keySource === signingKeyProvider.providerId,
    "Step29 failed: key source should be the signing key provider."
  );

  const deterministic = canonicalJson(firstRun.arb) === canonicalJson(secondRun.arb);
  assert(deterministic, "Step29 failed: ARB output is not deterministic for fixed inputs.");
  ...
  assert(
    ...
    "Step29 failed: missing signingKeyProvider should be rejected in release-required mode."
  );
  ...
  assert(
    ...
    "Step29 failed: missing keyReleasePolicy/keyId should be rejected in release-required mode."
  );

  const artifact = {
    schemaVersion: "step29-released-signer-required-v1",
    inputs: {
      cvmInfoPath,
      cliAttestationPath,
    },
  ...
  console.log(`Saved Step 29 artifact: ${ARTIFACT_PATH}`);
  ...
  console.log("Step 29 released-signer-required test: PASS");
}

main().catch((error) => {
  console.error("Step 29 released-signer-required test failed.", error);
  process.exitCode = 1;
});
