...
const ARTIFACT_PATH = path.resolve(ROOT_DIR, "artifacts", "step55.phala-minimal-pinned-runtime-measurement.json");
...
const APP_PORT = 8822;
const CLOUD_VERSION = "2026-03-16-step55";
const CLOUD_API_KEY = "step55-cloud-api-key";
...
async function main() {
  const tempDir = path.resolve(ROOT_DIR, "artifacts", "step55-temp");
  await mkdir(tempDir, { recursive: true });
  const envFile = path.join(tempDir, "minimal.env");
  const relativeEnvFile = path.relative(ROOT_DIR, envFile).replace(/\\/g, "/");

  const initialEnv = [
    "PHALA_CVM_NAME=ln-advisor-phala-minimal-step55",
    "PROTOTYPE_SERVICE_NAME=ln-advisor-phala-minimal-step55",
  ...
  try {
    ...
    assert(
      pinExitCode === 0,
      `Step55 failed: pin script did not exit cleanly.\nstdout:\n${pinStdout}\nstderr:\n${pinStderr}`
    );
    ...
    assert(
      ...
      "Step55 failed: pin script did not update PROTOTYPE_VERIFY_ALLOWED_MEASUREMENT."
    );
    assert(
      parsedEnv.PROTOTYPE_ATTESTATION_MEASUREMENT === REAL_MEASUREMENT,
      "Step55 failed: pin script did not update PROTOTYPE_ATTESTATION_MEASUREMENT."
    );
    ...
    try {
      ...
      assert(health.status === 200, "Step55 failed: health should return 200.");
      assert(
        health.body?.measurementPolicy?.pinned === true,
        "Step55 failed: health should report measurement policy as pinned."
      );
      assert(
        health.body?.measurementPolicy?.allowedMeasurement === REAL_MEASUREMENT,
        "Step55 failed: health allowedMeasurement mismatch."
      );
      assert(
        health.body?.measurementPolicy?.placeholderDetected === false,
        "Step55 failed: health should not report placeholderDetected after pinning."
      );
      ...
      assert(
        ...
        "Step55 failed: rendered compose missing pinned verify measurement."
      );
      assert(
        renderedCompose.includes(`PROTOTYPE_ATTESTATION_MEASUREMENT: "${REAL_MEASUREMENT}"`),
        "Step55 failed: rendered compose missing pinned attestation measurement."
      );

      const artifact = {
        schemaVersion: "step55-phala-minimal-pinned-runtime-measurement-v1",
        envFile,
      ...
      console.log(`Saved Step 55 artifact: ${ARTIFACT_PATH}`);
      ...
      console.log("Step 55 phala minimal pinned runtime measurement test: PASS");
    } finally {
...
main().catch((error) => {
  console.error("Step 55 phala minimal pinned runtime measurement test failed.", error);
  process.exitCode = 1;
});
