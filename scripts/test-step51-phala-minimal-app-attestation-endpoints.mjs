...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "step51.phala-minimal-app-attestation-endpoints.json");
...
const PORT = 8815;
const TEST_QUOTE = "step51-quote-0123456789abcdef0123456789abcdef0123456789abcdef";
...
async function main() {
  const child = spawn(process.execPath, [SERVER_PATH], {
    ...
    env: {
      ...
      PROTOTYPE_SERVICE_NAME: "ln-advisor-phala-minimal-step51-test",
      PROTOTYPE_SIGNER_PROVIDER_ID: "prototype-env-signer-step51",
      PROTOTYPE_ATTESTATION_MEASUREMENT: "step51-measurement",
  ...
  try {
    ...
    assert(health.body.mode === "minimal_props_service", "Step51 failed: health mode mismatch.");
    assert(compactRecommend.status === 200, "Step51 failed: compact /api/recommend should return 200.");
    assert(compactInfo.status === 200, "Step51 failed: compact /info should return 200.");
    assert(compactAttestation.status === 200, "Step51 failed: compact /attestation should return 200.");
    assert(info.status === 200, "Step51 failed: /info should return 200.");
    assert(attestation.status === 200, "Step51 failed: /attestation should return 200.");
    assert(typeof appCompose === "string" && appCompose.length > 0, "Step51 failed: app_compose missing.");
    assert(
      compactInfo.body?.tcb_info?.app_compose === undefined,
      "Step51 failed: compact /info should omit full app_compose."
    );
    assert(
      typeof compactInfo.body?.tcb_info?.app_compose_preview === "string" &&
        compactInfo.body.tcb_info.app_compose_preview.includes("..."),
      "Step51 failed: compact /info should expose app_compose_preview."
    );
    assert(
      compactInfo.body?.tcb_info?.app_compose_length === appCompose.length,
      "Step51 failed: compact /info app_compose_length mismatch."
    );
    assert(
      compactInfo.body?.tcb_info?.compose_hash === composeHash,
      "Step51 failed: compact /info compose_hash mismatch."
    );
    assert(attestation.body.quote === TEST_QUOTE, "Step51 failed: live attestation quote mismatch.");
    assert(
      compactAttestation.body?.quote === undefined,
      "Step51 failed: compact /attestation should omit full quote."
    );
    assert(
      typeof compactAttestation.body?.quote_preview === "string" &&
        compactAttestation.body.quote_preview.includes("..."),
      "Step51 failed: compact /attestation should expose quote_preview."
    );
    assert(
      compactAttestation.body?.quote_length === TEST_QUOTE.length,
      "Step51 failed: compact /attestation quote_length mismatch."
    );
    assert(
      compactRecommend.body?.arb?.attestation?.quote === undefined,
      "Step51 failed: compact /api/recommend should omit full ARB quote."
    );
    assert(
      ...
      "Step51 failed: compact /api/recommend should expose quote_preview."
    );
    assert(attestation.body.report_data === recommend.body.arb.digest, "Step51 failed: live attestation report_data should match latest digest.");
    assert(eventDigest === composeHash, "Step51 failed: event log compose hash mismatch.");
    assert(info.body.signerProvider?.providerId === "prototype-env-signer-step51", "Step51 failed: /info signerProviderId mismatch.");

    const composeSource = await readFile(COMPOSE_PATH, "utf8");
    assert(composeSource.includes('url.pathname === "/info"'), "Step51 failed: compose scaffold missing /info route.");
    assert(composeSource.includes('url.pathname === "/attestation"'), "Step51 failed: compose scaffold missing /attestation route.");

    const artifact = {
      schemaVersion: "step51-phala-minimal-app-attestation-endpoints-v1",
      health: health.body,
...
main().catch((error) => {
  console.error("Step 51 phala minimal app attestation endpoints test failed.", error);
  process.exitCode = 1;
});
