...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "step17.phala-cloud-bootstrap.json");
...
async function main(): Promise<void> {
  ...
  assert(user.data.schemaVersion === "current-user-v20260121", "Step17 failed: current-user schema parse mismatch.");
  assert(cvmInfo.data.schemaVersion === "cvm-info-v20260121", "Step17 failed: cvm-info schema parse mismatch.");
  assert(Boolean(commit.data.id), "Step17 failed: commit flow did not return cvm id.");
  assert(
    encryptedEnvUpdate.data.encryptedEnvBundle.schemaVersion === "phala-encrypted-env-v1",
    "Step17 failed: encrypted env bundle schema mismatch."
  );
  assert(!!composeUpdate.data, "Step17 failed: compose update response missing.");
  assert(cvmAttestation.data.quote === quoteHex, "Step17 failed: CVM attestation quote mismatch.");
  assert(verifierResult.ok, "Step17 failed: external verifier returned not-ok.");
  ...
  for (const call of cloudCalls) {
    assert(
      ...
      `Step17 failed: missing ${PHALA_API_VERSION_HEADER} header on ${call.method} ${call.path}.`
    );
  }
  ...
  for (const required of expectedFlow) {
    assert(calledPaths.includes(required), `Step17 failed: missing expected API call ${required}.`);
  }

  const artifact = {
    schemaVersion: "step17-phala-cloud-bootstrap-v1",
    api: {
  ...
  console.log(`Saved Step 17 artifact: ${ARTIFACT_PATH}`);
  console.log(`API calls captured: ${calledPaths.length}`);
  console.log(`Verifier status: ${verifierResult.ok ? "PASS" : "FAIL"}`);
  console.log("Step 17 phala cloud bootstrap test: PASS");
}

main().catch((error) => {
  console.error("Step 17 phala cloud bootstrap test failed.", error);
  process.exitCode = 1;
});
