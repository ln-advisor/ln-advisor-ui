...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "step28.verify-source-binding.json");
...
const FIXED_DEV_SIGNING_KEY = "step28-dev-signing-key";
...
async function main(): Promise<void> {
  ...
  assert(verifyArbResult.ok, `Step28 verifyArb failed: ${verifyArbResult.errors.join(" | ")}`);
  ...
  assert(bindingPass.ok, `Step28 binding should pass: ${bindingPass.errors.join(" | ")}`);
  ...
  assert(!bindingWrongSource.ok, "Step28 failed: mismatched source verification should be rejected.");

  const bindingMissing = verifySourceVerificationBinding({
    sourceProvenance: run.sourceProvenance,
    requireSourceVerification: true,
  });
  assert(!bindingMissing.ok, "Step28 failed: missing source verification should be rejected.");
  ...
  for (const call of cloudCalls) {
    assert(
      ...
      `Step28 failed: missing ${PHALA_API_VERSION_HEADER} header on ${call.method} ${call.path}.`
    );
  }

  const artifact = {
    schemaVersion: "step28-verify-source-binding-v1",
    inputs: {
      cvmInfoPath,
      cliAttestationPath,
    },
  ...
  console.log(`Saved Step 28 artifact: ${ARTIFACT_PATH}`);
  ...
  console.log("Step 28 verify source-binding test: PASS");
}

main().catch((error) => {
  console.error("Step 28 verify source-binding test failed.", error);
  process.exitCode = 1;
});
