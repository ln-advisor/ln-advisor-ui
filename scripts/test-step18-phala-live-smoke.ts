...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "step18.phala-live-smoke.json");
...
async function main(): Promise<void> {
  ...
  const artifact = {
    schemaVersion: "step18-phala-live-smoke-v1",
    runMode: {
      readOnly: true,
  ...
  console.log(`Saved Step 18 artifact: ${ARTIFACT_PATH}`);
  ...
  console.log(`Step 18 live smoke test: ${overallOk ? "PASS" : "FAIL"}`);
...
main().catch((error) => {
  console.error("Step 18 live smoke test failed.", error);
  process.exitCode = 1;
});
