...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "step26.pipeline-source-verification-gate.json");
...
const FIXED_DEV_SIGNING_KEY = "step26-dev-signing-key";
...
async function main(): Promise<void> {
  ...
  assert(verifyPassRun.ok, `Step26 verifyArb failed: ${verifyPassRun.errors.join(" | ")}`);
  assert(passRun.runSummary.sourceVerificationGate.policyApplied, "Step26 failed: gate should be policy-applied.");
  assert(passRun.runSummary.sourceVerificationGate.verified, "Step26 failed: gate should be verified.");
  ...
  assert(deterministic, "Step26 failed: gated pipeline output is not deterministic for fixed inputs.");
  ...
  assert(
    ...
    "Step26 failed: missing source verification should be rejected."
  );
  ...
  assert(
    ...
    "Step26 failed: disallowed verification source should be rejected."
  );
  ...
  for (const call of cloudCalls) {
    assert(
      ...
      `Step26 failed: missing ${PHALA_API_VERSION_HEADER} header on ${call.method} ${call.path}.`
    );
  }

  const artifact = {
    schemaVersion: "step26-pipeline-source-verification-gate-v1",
    inputs: {
      cvmInfoPath,
      cliAttestationPath,
    },
  ...
  console.log(`Saved Step 26 artifact: ${ARTIFACT_PATH}`);
  ...
  console.log("Step 26 pipeline source verification gate test: PASS");
}

main().catch((error) => {
  console.error("Step 26 pipeline source verification gate test failed.", error);
  process.exitCode = 1;
});
