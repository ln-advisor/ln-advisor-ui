...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "step50.phala-minimal-rendered-compose.json");
...
const TEMP_ENV_PATH = path.resolve(process.cwd(), "artifacts", "step50.render.env");
...
async function main() {
  ...
  assert(exitCode === 0, "Step50 failed: sync script did not exit cleanly.");
  ...
  assert(templateCompose.includes('${PROTOTYPE_SIGNER_PROVIDER_ID:-prototype-env-signer-v1}'), "Step50 failed: template compose should keep signer provider placeholder.");
  assert(renderedCompose.includes('rendered-env-signer-v1'), "Step50 failed: rendered compose missing concrete signer provider value.");
  assert(renderedCompose.includes('rendered-shell-measurement-v1'), "Step50 failed: rendered compose missing concrete attestation measurement value.");
  assert(renderedCompose.includes('PROTOTYPE_VERIFY_REQUIRE_SOURCE_RECEIPT: "true"'), "Step50 failed: rendered compose missing concrete source receipt policy value.");
  assert(
    renderedCompose.includes('PHALA_CLOUD_API_KEY: ""'),
    "Step50 failed: rendered compose should still resolve empty-string env placeholders."
  );
  assert(!renderedCompose.includes('${PROTOTYPE_SIGNER_PROVIDER_ID:-prototype-env-signer-v1}'), "Step50 failed: rendered compose should not keep unresolved signer provider placeholder.");
  assert(
    !renderedCompose.includes('rendered-env-file-signer-v1'),
    "Step50 failed: shell env should override file signer provider value."
  );

  const artifact = {
    schemaVersion: "step50-phala-minimal-rendered-compose-v1",
    templateChecks: {
    ...
    renderedChecks: {
      ...
      envFileLoaded: stdout.includes('step50.render.env'),
    },
    process: {
      stdout: stdout.trim(),
      stderr: stderr.trim(),
  ...
  console.log(`Saved Step 50 artifact: ${ARTIFACT_PATH}`);
  ...
  console.log("Step 50 phala minimal rendered compose test: PASS");
}

main().catch((error) => {
  console.error("Step 50 phala minimal rendered compose test failed.", error);
  process.exitCode = 1;
});
