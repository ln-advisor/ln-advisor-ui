...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "step21.key-release-signer.json");
...
async function main(): Promise<void> {
  ...
  const releasedKeyMaterial = "step21-released-signing-key-material";
  ...
  assert(verifyResult.ok, `Step21 verifyArb failed: ${verifyResult.errors.join(" | ")}`);
  assert(firstRun.runSummary.keyRelease.granted, "Step21 failed: key release should be granted.");
  assert(
    firstRun.runSummary.keyRelease.keyId === releasedKeyId,
    "Step21 failed: granted keyId does not match policy keyId."
  );
  assert(
    firstRun.runSummary.keyRelease.keySource === signingKeyProvider.providerId,
    "Step21 failed: key source should be the signing key provider."
  );

  const deterministic = canonicalJson(firstRun.arb) === canonicalJson(secondRun.arb);
  assert(deterministic, "Step21 failed: ARB output is not deterministic for fixed inputs.");
  ...
  assert(
    ...
    "Step21 failed: missing signer provider should be rejected."
  );

  const artifact = {
    schemaVersion: "step21-key-release-signer-v1",
    inputs: {
      cvmInfoPath,
      cliAttestationPath,
    },
  ...
  console.log(`Saved Step 21 artifact: ${ARTIFACT_PATH}`);
  ...
  console.log("Step 21 key-release signer test: PASS");
}

main().catch((error) => {
  console.error("Step 21 key-release signer test failed.", error);
  process.exitCode = 1;
});
