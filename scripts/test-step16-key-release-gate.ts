...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "step16.key-release-gate.json");
const FIXED_DEV_SIGNING_KEY = "step16-dev-signing-key";
...
async function main(): Promise<void> {
  ...
  assert(verifiedArbCheck.ok, `Step16 verified path failed ARB verification: ${verifiedArbCheck.errors.join(" | ")}`);
  assert(
    verifiedRun.runSummary.keyRelease.policyApplied && verifiedRun.runSummary.keyRelease.granted,
    "Step16 verified path did not report granted key release."
  );
  ...
  assert(deniedMessage.includes("Key release denied:"), "Step16 expected simulated path key release denial.");
  ...
  assert(deterministic, "Step16 failed: verified path ARB output is not deterministic.");

  const artifact = {
    schemaVersion: "step16-key-release-gate-v1",
    policy: strictPolicy,
    verifiedPath: {
  ...
  console.log(`Saved Step 16 artifact: ${ARTIFACT_PATH}`);
  ...
  console.log("Step 16 key-release gate test: PASS");
}

main().catch((error) => {
  console.error("Step 16 key-release gate test failed.", error);
  process.exitCode = 1;
});
