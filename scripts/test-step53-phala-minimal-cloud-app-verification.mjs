...
const ARTIFACT_PATH = path.resolve(process.cwd(), "artifacts", "step53.phala-minimal-cloud-app-verification.json");
...
const CLOUD_VERSION = "2026-03-14-step53";
const CLOUD_API_KEY = "step53-cloud-api-key";

const sampleTelemetry = {
  nodeAlias: "phala-step53-node",
  channels: [
    {
      channelId: "100x1x0",
      peerPubkey: "02aaaa",
...
async function runSyncWithRenderValues() {
  ...
  assert(exitCode === 0, "Step53 failed: sync script did not exit cleanly.");
  return {
    stdout: stdout.trim(),
    stderr: stderr.trim(),
  };
...
async function startPrototypeServer(port, attestationQuote) {
  const child = spawn(process.execPath, [SERVER_PATH], {
    ...
    env: {
      ...
      PROTOTYPE_SERVICE_NAME: `ln-advisor-phala-minimal-step53-${port}`,
      PROTOTYPE_ARB_SIGNING_KEY: "step53-signing-key",
      PROTOTYPE_SIGNER_PROVIDER_ID: "prototype-env-signer-step53",
      ...
      PROTOTYPE_ATTESTATION_MEASUREMENT: "step53-measurement",
      ...
      PROTOTYPE_VERIFY_ALLOWED_MEASUREMENT: "step53-measurement",
...
async function main() {
  ...
  const fakeCloudServer = http.createServer(async (req, res) => {
    if (req.method === "POST" && req.url === "/attestations/verify") {
      ...
      const quoteVerified = quoteHex === "step53-quote";
      const payload = {
        success: true,
        quote: {
  ...
  const goodServer = await startPrototypeServer(GOOD_PORT, "step53-quote");
  const badServer = await startPrototypeServer(BAD_PORT, "step53-quote-unverified");
  ...
  try {
    ...
    assert(goodRecommend.status === 200, "Step53 failed: good recommend should return 200.");
    assert(goodVerify.body.ok === true, "Step53 failed: verified quote should pass cloud app verification.");
    assert(goodVerify.body.cloudVerification?.quoteVerified === true, "Step53 failed: good quote should be marked verified.");
    assert(
      goodVerify.body.cloudVerification?.raw?.quote?.verified === true,
      "Step53 failed: compact verify response should keep cloud quote verification status."
    );
    assert(
      goodVerify.body.cloudVerification?.raw?.quote?.hex === undefined,
      "Step53 failed: compact verify response should omit the raw cloud quote hex."
    );
    assert(
      goodVerify.body.cloudVerification?.raw?.quote_collateral === undefined,
      "Step53 failed: compact verify response should omit cloud quote collateral blobs."
    );
    assert(
      goodVerify.body.liveAppEvidencePolicy?.requireCloudAppVerification === true,
      "Step53 failed: live app evidence policy should require cloud verification."
    );

    assert(badVerify.body.ok === false, "Step53 failed: unverified quote should fail verification.");
    assert(
      badVerify.body.errors.includes("liveAppAttestation.quote verification via Phala cloud API failed."),
      "Step53 failed: missing cloud quote verification failure error."
    );
    assert(badVerify.body.cloudVerification?.quoteVerified === false, "Step53 failed: bad quote should be marked unverified.");

    assert(cloudCalls.length === 2, "Step53 failed: fake Phala cloud API should receive two verification calls.");
    for (const call of cloudCalls) {
      assert(call.headers["x-api-key"] === CLOUD_API_KEY, "Step53 failed: missing X-API-Key header.");
      assert(call.headers["x-phala-version"] === CLOUD_VERSION, "Step53 failed: missing X-Phala-Version header.");
    }
    assert(cloudCalls[0].body?.hex === "step53-quote", "Step53 failed: first cloud verification should receive the good quote.");
    assert(
      cloudCalls[1].body?.hex === "step53-quote-unverified",
      "Step53 failed: second cloud verification should receive the unverified quote."
    );
    ...
    assert(
      ...
      "Step53 failed: template compose missing cloud app verification placeholder."
    );
    assert(
      templateCompose.includes('${PHALA_CLOUD_API_BASE_URL:-https://cloud-api.phala.network/api/v1}'),
      "Step53 failed: template compose missing Phala API base URL placeholder."
    );
    assert(
      templateCompose.includes('${PHALA_API_VERSION:-2026-01-21}'),
      "Step53 failed: template compose missing Phala API version placeholder."
    );
    assert(
      templateCompose.includes('${PHALA_CLOUD_API_KEY:-}'),
      "Step53 failed: template compose missing Phala API key placeholder."
    );
    assert(
      renderedCompose.includes(`PROTOTYPE_VERIFY_REQUIRE_CLOUD_APP_VERIFICATION: "true"`),
      "Step53 failed: rendered compose missing concrete cloud verification policy value."
    );
    assert(
      renderedCompose.includes(`PHALA_CLOUD_API_BASE_URL: "http://127.0.0.1:${CLOUD_PORT}"`),
      "Step53 failed: rendered compose missing concrete Phala API base URL."
    );
    assert(
      renderedCompose.includes(`PHALA_API_VERSION: "${CLOUD_VERSION}"`),
      "Step53 failed: rendered compose missing concrete Phala API version."
    );
    assert(
      renderedCompose.includes(`PHALA_CLOUD_API_KEY: "${CLOUD_API_KEY}"`),
      "Step53 failed: rendered compose missing concrete Phala API key."
    );

    const artifact = {
      schemaVersion: "step53-phala-minimal-cloud-app-verification-v1",
      good: {
    ...
    console.log(`Saved Step 53 artifact: ${ARTIFACT_PATH}`);
    ...
    console.log("Step 53 phala minimal cloud app verification test: PASS");
  } finally {
...
main().catch((error) => {
  console.error("Step 53 phala minimal cloud app verification test failed.", error);
  process.exitCode = 1;
});
