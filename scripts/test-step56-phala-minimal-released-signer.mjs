...
const ARTIFACT_PATH = path.resolve(ROOT_DIR, "artifacts", "step56.phala-minimal-released-signer.json");
...
const RELEASED_KEY_ID = "arb-signer-step56";
const RELEASED_KEY_PROVIDER_ID = "prototype-key-release-provider-step56";
const RELEASED_SIGNER_PROVIDER_ID = "prototype-released-signer-step56";
const RELEASED_KEY_MATERIAL = "step56-released-signing-key";
const GOOD_MEASUREMENT = "step56-measurement";
const BAD_MEASUREMENT = "step56-measurement-mismatch";

const sampleTelemetry = {
  nodeAlias: "phala-step56-node",
  channels: [
    {
      channelId: "100x1x0",
      peerPubkey: "02aaaa",
...
async function runSync(envFilePath) {
  ...
  assert(exitCode === 0, `Step56 failed: sync script exited with ${exitCode}.\nstdout:\n${stdout}\nstderr:\n${stderr}`);
...
async function main() {
  const tempDir = path.resolve(ROOT_DIR, "artifacts", "step56-temp");
  await mkdir(tempDir, { recursive: true });
  const envFile = path.join(tempDir, "minimal.env");

  const envContents = [
    "PHALA_CVM_NAME=ln-advisor-phala-minimal-step56",
    "PROTOTYPE_SERVICE_NAME=ln-advisor-phala-minimal-step56",
    ...
    "PROTOTYPE_ATTESTATION_QUOTE=step56-phala-quote",
    "PROTOTYPE_ATTESTATION_NONCE=step56-attestation-nonce",
  ...
  try {
    ...
    assert(health.status === 200, "Step56 failed: /health should return 200.");
    assert(
      health.body?.signerProvider?.providerType === "released_keyring",
      "Step56 failed: /health signerProvider.providerType mismatch."
    );
    assert(
      health.body?.signerProvider?.signingMode === "released_signer",
      "Step56 failed: /health signingMode should report released_signer."
    );
    assert(
      health.body?.signerProvider?.keyProviderId === RELEASED_KEY_PROVIDER_ID,
      "Step56 failed: /health keyProviderId mismatch."
    );
    assert(
      health.body?.signerProvider?.releasedSignerPolicy?.keyId === RELEASED_KEY_ID,
      "Step56 failed: /health released signer policy keyId mismatch."
    );

    assert(recommend.status === 200, "Step56 failed: released signer recommend should return 200.");
    assert(recommend.body?.signingMode === "released_signer", "Step56 failed: signingMode should be released_signer.");
    assert(
      recommend.body?.arb?.signature?.signerProviderId === RELEASED_SIGNER_PROVIDER_ID,
      "Step56 failed: signature signerProviderId mismatch."
    );
    assert(
      recommend.body?.arb?.signature?.signerProviderType === "released_keyring",
      "Step56 failed: signature signerProviderType mismatch."
    );
    assert(
      recommend.body?.arb?.signature?.keyId === RELEASED_KEY_ID,
      "Step56 failed: signature keyId mismatch."
    );
    assert(
      recommend.body?.arb?.signature?.keySource === RELEASED_KEY_PROVIDER_ID,
      "Step56 failed: signature keySource should come from released signer key provider."
    );
    assert(
      recommend.body?.arb?.attestation?.measurement === GOOD_MEASUREMENT,
      "Step56 failed: attestation measurement mismatch."
    );
    assert(verify.body?.ok === true, "Step56 failed: released signer verification should pass.");
    assert(
      verify.body?.signerPolicy?.signingMode === "released_signer",
      "Step56 failed: verify signerPolicy signingMode mismatch."
    );
    assert(
      verify.body?.signerPolicy?.releasedSignerPolicy?.keyId === RELEASED_KEY_ID,
      "Step56 failed: verify signerPolicy missing released signer policy."
    );

    assert(
      deniedRecommend.status === 503,
      "Step56 failed: released signer should fail closed when attestation policy denies key release."
    );
    assert(
      String(deniedRecommend.body?.error || "").includes("Released signer key release denied:"),
      "Step56 failed: denied recommend should report key release denial."
    );
    assert(
      String(deniedRecommend.body?.error || "").includes("Attestation measurement is not allowed for released signer key release."),
      "Step56 failed: denied recommend should report the attestation measurement policy violation."
    );
    ...
    assert(
      ...
      "Step56 failed: compose scaffold missing PROTOTYPE_RELEASED_SIGNER_KEY_ID."
    );
    assert(
      templateCompose.includes("PROTOTYPE_RELEASED_SIGNER_KEYRING_JSON"),
      "Step56 failed: compose scaffold missing PROTOTYPE_RELEASED_SIGNER_KEYRING_JSON."
    );
    assert(
      templateCompose.includes("PROTOTYPE_RELEASED_SIGNER_ALLOWED_MEASUREMENTS"),
      "Step56 failed: compose scaffold missing PROTOTYPE_RELEASED_SIGNER_ALLOWED_MEASUREMENTS."
    );
    assert(
      renderedCompose.includes(`PROTOTYPE_SIGNER_PROVIDER_TYPE: "released_keyring"`),
      "Step56 failed: rendered compose missing released_keyring provider type."
    );
    assert(
      renderedCompose.includes(`PROTOTYPE_RELEASED_SIGNER_KEY_PROVIDER_ID: "${RELEASED_KEY_PROVIDER_ID}"`),
      "Step56 failed: rendered compose missing released signer key provider id."
    );

    const artifact = {
      schemaVersion: "step56-phala-minimal-released-signer-v1",
      health: health.body,
    ...
    console.log(`Saved Step 56 artifact: ${ARTIFACT_PATH}`);
    ...
    console.log("Step 56 phala minimal released signer test: PASS");
  } finally {
...
main().catch((error) => {
  console.error("Step 56 phala minimal released signer test failed.", error);
  process.exitCode = 1;
});
