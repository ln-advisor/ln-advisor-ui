...
function parseJsonObjectEnv(value, envName) {
  let parsed;
  try {
  ...
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error(`${envName} must be a JSON object map of keyId -> keyMaterial.`);
  }

  const keyring = {};
  for (const [key, rawValue] of Object.entries(parsed)) {
    const keyId = String(key || "").trim();
    const keyMaterial = typeof rawValue === "string" ? rawValue.trim() : "";
    if (!keyId || !keyMaterial) continue;
    keyring[keyId] = keyMaterial;
  }

  if (Object.keys(keyring).length === 0) {
    throw new Error(`${envName} must contain at least one non-empty keyId -> keyMaterial pair.`);
  }
...
const PROTOTYPE_RELEASED_SIGNER_KEY_ID =
  (process.env.PROTOTYPE_RELEASED_SIGNER_KEY_ID || "").trim();
const PROTOTYPE_RELEASED_SIGNER_KEY_PROVIDER_ID =
  (process.env.PROTOTYPE_RELEASED_SIGNER_KEY_PROVIDER_ID || "prototype-released-signing-key-provider-v1").trim();
const PROTOTYPE_RELEASED_SIGNER_KEYRING_JSON =
  (process.env.PROTOTYPE_RELEASED_SIGNER_KEYRING_JSON || "").trim();
const PROTOTYPE_RELEASED_SIGNER_REQUIRE_ATTESTATION =
  process.env.PROTOTYPE_RELEASED_SIGNER_REQUIRE_ATTESTATION !== "false";
const PROTOTYPE_RELEASED_SIGNER_MIN_EXECUTION_MODE =
  (process.env.PROTOTYPE_RELEASED_SIGNER_MIN_EXECUTION_MODE || "tee_verified").trim();
const PROTOTYPE_RELEASED_SIGNER_ALLOWED_PROVIDER_IDS_RAW =
  (process.env.PROTOTYPE_RELEASED_SIGNER_ALLOWED_PROVIDER_IDS || "").trim();
const PROTOTYPE_RELEASED_SIGNER_ALLOWED_MEASUREMENTS_RAW =
  (process.env.PROTOTYPE_RELEASED_SIGNER_ALLOWED_MEASUREMENTS || "").trim();
const PROTOTYPE_RELEASED_SIGNER_ALLOWED_QUOTE_FORMATS_RAW =
  (process.env.PROTOTYPE_RELEASED_SIGNER_ALLOWED_QUOTE_FORMATS || "").trim();
...
const PROTOTYPE_RELEASED_SIGNER_ALLOWED_PROVIDER_IDS =
  parseCsvEnv(PROTOTYPE_RELEASED_SIGNER_ALLOWED_PROVIDER_IDS_RAW) || [PROTOTYPE_ATTESTATION_PROVIDER_ID];
const PROTOTYPE_RELEASED_SIGNER_ALLOWED_MEASUREMENTS =
  parseCsvEnv(PROTOTYPE_RELEASED_SIGNER_ALLOWED_MEASUREMENTS_RAW) || [PROTOTYPE_VERIFY_ALLOWED_MEASUREMENT];
const PROTOTYPE_RELEASED_SIGNER_ALLOWED_QUOTE_FORMATS =
  parseCsvEnv(PROTOTYPE_RELEASED_SIGNER_ALLOWED_QUOTE_FORMATS_RAW) || [PROTOTYPE_ATTESTATION_QUOTE_FORMAT];
...
const wantsDebugTrace = (url) => url.searchParams.get("debug") === "true";
...
const buildReleasedSignerPolicy = () => ({
  schemaVersion: "prototype-released-signer-policy-v1",
  keyId: PROTOTYPE_RELEASED_SIGNER_KEY_ID,
  requireAttestation: PROTOTYPE_RELEASED_SIGNER_REQUIRE_ATTESTATION,
  minExecutionMode: PROTOTYPE_RELEASED_SIGNER_MIN_EXECUTION_MODE,
  allowedProviderIds: [...PROTOTYPE_RELEASED_SIGNER_ALLOWED_PROVIDER_IDS],
  allowedMeasurements: [...PROTOTYPE_RELEASED_SIGNER_ALLOWED_MEASUREMENTS],
  allowedQuoteFormats: [...PROTOTYPE_RELEASED_SIGNER_ALLOWED_QUOTE_FORMATS],
});

const evaluateReleasedSignerPolicy = (attestation, policy) => {
  const errors = [];
  const warnings = [];

  if (!attestation || typeof attestation !== "object") {
    if (policy.requireAttestation || modeRank(policy.minExecutionMode) > 0) {
      errors.push("Attestation evidence is required for released signer key release but missing.");
    } else {
      warnings.push("Released signer key was released without attestation due to policy configuration.");
    }
  ...
  if (modeRank(attestation.executionMode) < modeRank(policy.minExecutionMode)) {
    errors.push(
      `Execution mode ${attestation.executionMode} does not satisfy minimum ${policy.minExecutionMode} for released signer key release.`
    );
  }

  if (policy.allowedProviderIds.length > 0 && !policy.allowedProviderIds.includes(attestation.providerId)) {
    errors.push(`Provider ${attestation.providerId} is not allowed for released signer key release.`);
  }

  if (policy.allowedMeasurements.length > 0 && !policy.allowedMeasurements.includes(attestation.measurement)) {
    errors.push("Attestation measurement is not allowed for released signer key release.");
  }

  if (policy.allowedQuoteFormats.length > 0 && !policy.allowedQuoteFormats.includes(attestation.quoteFormat)) {
    errors.push(`Quote format ${attestation.quoteFormat} is not allowed for released signer key release.`);
  }
...
const percentile = (sortedValues, quantile) => {
...
const buildPotentialCentralityThresholds = (potentialPeers) => {
  const values = (Array.isArray(potentialPeers) ? potentialPeers : [])
    .map((peer) => (peer?.betweennessCentrality === null || peer?.betweennessCentrality === undefined
      ? null
      : Number(peer.betweennessCentrality)))
...
const classifyCentralityBand = (centrality, thresholds) => {
  if (centrality === null || centrality === undefined) return "MEDIUM";
  if (centrality <= thresholds.p33) return "LOW";
  if (centrality <= thresholds.p66) return "MEDIUM";
  return "HIGH";
};

const classifyMissionReliabilityBand = (successRate, failureRate) => {
...
const classifyCapacityBand = (capacitySat) => {
  if (capacitySat > 10_000_000) return "HIGH";
  if (capacitySat > 1_000_000) return "MEDIUM";
  return "LOW";
};
...
const isFeatureOnlyNodeState = (value) =>
  Boolean(value) &&
  typeof value === "object" &&
  value.privacyMode === "feature_only" &&
...
const toFeatureOnly = (telemetry) => {
  if (isFeatureOnlyNodeState(telemetry)) {
    const channels = [...telemetry.channels]
      .map((channel, index) => ({
        channelRef: String(channel?.channelRef || makeRef("channel", index)),
        peerRef: String(channel?.peerRef || makeRef("peer", index)),
        active: Boolean(channel?.active),
        localBalanceRatio: roundFixed(Number(channel?.localBalanceRatio || 0), 6),
        remoteBalanceRatio: roundFixed(Number(channel?.remoteBalanceRatio || 0), 6),
        outboundFeePpm: normalizeNullableNumber(channel?.outboundFeePpm),
        forwardCount:
          Number(
            channel?.forwardCount ??
              channel?.forwardCountTotal ??
              (Number(channel?.forwardCountIn || 0) + Number(channel?.forwardCountOut || 0))
          ) || 0,
        revenueSat: Number(channel?.revenueSat || 0),
        failedForwardCount: Number(channel?.failedForwardCount || 0),
        peerBetweennessCentrality: normalizeNullableNumber(channel?.peerBetweennessCentrality),
        missionSuccessRate: normalizeNullableNumber(channel?.missionSuccessRate),
        missionFailureRate: normalizeNullableNumber(channel?.missionFailureRate),
      }))
      .sort((left, right) => compareText(left.channelRef, right.channelRef));

    const peers = [...(Array.isArray(telemetry.peers) ? telemetry.peers : [])]
      .map((peer, index) => ({
        peerRef: String(peer?.peerRef || makeRef("peer", index)),
        channelCount: Number(peer?.channelCount || 0),
        activeChannelCount: Number(peer?.activeChannelCount || 0),
        avgLocalBalanceRatio: roundFixed(Number(peer?.avgLocalBalanceRatio || 0), 6),
        avgRemoteBalanceRatio: roundFixed(Number(peer?.avgRemoteBalanceRatio || 0), 6),
        avgOutboundFeePpm: normalizeNullableNumber(peer?.avgOutboundFeePpm),
        totalForwardCount: Number(peer?.totalForwardCount || 0),
        totalRevenueSat: Number(peer?.totalRevenueSat || 0),
        totalFailedForwardCount: Number(peer?.totalFailedForwardCount || 0),
        lastActivityTimestamp: normalizeNullableNumber(peer?.lastActivityTimestamp),
        avgPeerBetweennessCentrality: normalizeNullableNumber(peer?.avgPeerBetweennessCentrality),
        missionPairCount: Number(peer?.missionPairCount || 0),
        missionSuccessRate: normalizeNullableNumber(peer?.missionSuccessRate),
        missionFailureRate: normalizeNullableNumber(peer?.missionFailureRate),
      }))
      .sort((left, right) => compareText(left.peerRef, right.peerRef));

    const potentialPeers = [...(Array.isArray(telemetry.potentialPeers) ? telemetry.potentialPeers : [])]
      .map((peer, index) => ({
        peerRef: String(peer?.peerRef || makeRef("peer", peers.length + index)),
        capacitySat: Number(peer?.capacitySat || 0),
        channelCount: Number(peer?.channelCount || 0),
        betweennessCentrality: normalizeNullableNumber(peer?.betweennessCentrality),
        missionSuccessRate: normalizeNullableNumber(peer?.missionSuccessRate),
        missionFailureRate: normalizeNullableNumber(peer?.missionFailureRate),
        lastActivityTimestamp: normalizeNullableNumber(peer?.lastActivityTimestamp),
      }))
    ...
    const totals = isRecord(telemetry.totals)
      ? {
          forwardCount: Number(telemetry.totals.forwardCount || 0),
          revenueSat: Number(telemetry.totals.revenueSat || 0),
          failedForwardCount: Number(telemetry.totals.failedForwardCount || 0),
          avgLocalBalanceRatio: roundFixed(Number(telemetry.totals.avgLocalBalanceRatio || 0), 6),
          avgRemoteBalanceRatio: roundFixed(Number(telemetry.totals.avgRemoteBalanceRatio || 0), 6),
          missionPairCount: Number(telemetry.totals.missionPairCount || 0),
          missionPairsWithSignals: Number(telemetry.totals.missionPairsWithSignals || 0),
          centralityPeerCount: Number(telemetry.totals.centralityPeerCount || 0),
        }
      : {
          forwardCount: channels.reduce((sum, channel) => sum + channel.forwardCount, 0),
          revenueSat: channels.reduce((sum, channel) => sum + channel.revenueSat, 0),
          failedForwardCount: channels.reduce((sum, channel) => sum + channel.failedForwardCount, 0),
          avgLocalBalanceRatio:
            channels.length > 0
              ? roundFixed(channels.reduce((sum, channel) => sum + channel.localBalanceRatio, 0) / channels.length, 6)
              : 0,
          avgRemoteBalanceRatio:
            channels.length > 0
              ? roundFixed(channels.reduce((sum, channel) => sum + channel.remoteBalanceRatio, 0) / channels.length, 6)
              : 0,
          missionPairCount: peers.reduce((sum, peer) => sum + Number(peer.missionPairCount || 0), 0),
          missionPairsWithSignals: peers.filter((peer) => peer.missionSuccessRate !== null || peer.missionFailureRate !== null).length,
          centralityPeerCount: potentialPeers.filter((peer) => peer.betweennessCentrality !== null).length,
        };

    return {
      schemaVersion: String(telemetry.schemaVersion || "privacy-node-state-v1"),
      privacyMode: "feature_only",
      sourceSchemaVersion: String(telemetry.sourceSchemaVersion || "privacy-node-state-v1"),
      nodeAlias: String(telemetry.nodeAlias || "prototype-node"),
      channelCount: Number(telemetry.channelCount || channels.length),
      channels,
      peers,
      potentialPeers,
      totals,
  ...
  return {
    ...
    potentialPeers: [],
    totals: {
      forwardCount: normalized.channels.reduce((sum, channel) => sum + channel.forwardCount, 0),
      revenueSat: normalized.channels.reduce((sum, channel) => sum + channel.revenueSat, 0),
      failedForwardCount: normalized.channels.reduce((sum, channel) => sum + channel.failedForwardCount, 0),
      avgLocalBalanceRatio:
        normalized.channels.length > 0
          ? roundFixed(
              normalized.channels.reduce((sum, channel) => sum + channel.localBalanceRatio, 0) / normalized.channels.length,
              6
            )
          : 0,
      avgRemoteBalanceRatio:
        normalized.channels.length > 0
          ? roundFixed(
              normalized.channels.reduce((sum, channel) => sum + channel.remoteBalanceRatio, 0) / normalized.channels.length,
              6
            )
          : 0,
      missionPairCount: 0,
...
const buildPrototypeAppCompose = () => ({
  ...
  signerProvider: buildSignerProviderStatus(),
  attestationDefaults: {
...
const buildFeeRecommendation = (channel) => {
  ...
  let confidence = 0.6;
  let matchedRule = "hold_balanced_channel";
  const checks = {
    forwardCountGte5: Number(channel.forwardCount || 0) >= 5,
    forwardCountGte8: Number(channel.forwardCount || 0) >= 8,
    localBalanceRatioGte0_7: Number(channel.localBalanceRatio || 0) >= 0.7,
    failedForwardCountEq0: Number(channel.failedForwardCount || 0) === 0,
    failedForwardCountGt0: Number(channel.failedForwardCount || 0) > 0,
    localBalanceRatioLte0_25: Number(channel.localBalanceRatio || 0) <= 0.25,
    forwardCountEq0: Number(channel.forwardCount || 0) === 0,
  };
  ...
  if (channel.forwardCount >= 5 && channel.localBalanceRatio >= 0.7 && channel.failedForwardCount === 0) {
    ...
    confidence = 0.86;
    matchedRule = channel.forwardCount >= 8 ? "raise_high_outbound_very_active" : "raise_high_outbound_active";
    ...
    matchedRule =
      channel.failedForwardCount > 0
        ? "lower_failed_forward_pressure"
        : channel.localBalanceRatio <= 0.25
          ? "lower_low_outbound_liquidity"
          : "lower_no_recent_forward_activity";
  ...
  return {
    ...
    debug: {
      ...
      inputs: {
        currentFeePpm,
        localBalanceSat: Number(channel.localBalanceSat || 0),
        remoteBalanceSat: Number(channel.remoteBalanceSat || 0),
        localBalanceRatio: Number(channel.localBalanceRatio || 0),
        remoteBalanceRatio: Number(channel.remoteBalanceRatio || 0),
        forwardCount: Number(channel.forwardCount || 0),
        failedForwardCount: Number(channel.failedForwardCount || 0),
        revenueSat: Number(channel.revenueSat || 0),
      },
      checks,
      matchedRule,
      deltaPpm: Number(suggestedFeePpm) - Number(currentFeePpm),
      output: recommendation,
    },
  };
};

const buildChannelOpeningRecommendation = (potentialPeer, centralityThresholds) => {
  const reasons = [];
  const centralityBand = classifyCentralityBand(potentialPeer.betweennessCentrality, centralityThresholds);
  const reliabilityBand = classifyMissionReliabilityBand(
    potentialPeer.missionSuccessRate,
    potentialPeer.missionFailureRate
  );
  const capacityBand = classifyCapacityBand(Number(potentialPeer.capacitySat || 0));

  const bc = potentialPeer.betweennessCentrality ?? 0;
  const capacityBtc = Math.max(1, Number(potentialPeer.capacitySat || 0) / 100_000_000);
  const reliabilityWeight =
    reliabilityBand === "HIGH" ? 1.5 : reliabilityBand === "LOW" ? 0.5 : 1.0;
  const rawScore = (bc / capacityBtc) * reliabilityWeight;
  ...
  if (capacityBand === "LOW" && Number(potentialPeer.channelCount || 0) > 5) {
    reasons.push("altruistic_liquidity_gap");
  }
  if (Number(potentialPeer.channelCount || 0) > 50) reasons.push("well_connected_hub");

  const recommendation = {
    peerRef: potentialPeer.peerRef,
    score,
    ...
    signals: {
      ...
      reliabilityBand,
      capacityBand,
      channelCount: Number(potentialPeer.channelCount || 0),
    },
  ...
  return {
    ...
    debug: {
      peerRef: potentialPeer.peerRef,
      inputs: {
        capacitySat: Number(potentialPeer.capacitySat || 0),
        channelCount: Number(potentialPeer.channelCount || 0),
        betweennessCentrality: Number(potentialPeer.betweennessCentrality || 0),
        missionSuccessRate:
          potentialPeer.missionSuccessRate === null || potentialPeer.missionSuccessRate === undefined
            ? null
            : Number(potentialPeer.missionSuccessRate),
        missionFailureRate:
          potentialPeer.missionFailureRate === null || potentialPeer.missionFailureRate === undefined
            ? null
            : Number(potentialPeer.missionFailureRate),
      },
      thresholds: {
        centralityP33: centralityThresholds?.p33 ?? null,
        centralityP66: centralityThresholds?.p66 ?? null,
      },
      derived: {
        centralityBand,
        reliabilityBand,
        ...
        capacityBtc,
        reliabilityWeight,
        rawScore: roundFixed(rawScore, 6),
      },
      matchedSignals: {
        highNetworkCentrality: centralityBand === "HIGH",
        provenRoutingReliability: reliabilityBand === "HIGH",
        altruisticLiquidityGap:
          capacityBand === "LOW" && Number(potentialPeer.channelCount || 0) > 5,
        wellConnectedHub: Number(potentialPeer.channelCount || 0) > 50,
      },
      output: recommendation,
    },
  };
...
const scorePrototypeRecommendations = (telemetry, options = {}) => {
  const debugEnabled = options?.debug === true;
  const transformedSnapshot = toFeatureOnly(telemetry);
  const feeResults = transformedSnapshot.channels.map(buildFeeRecommendation);
  const centralityThresholds = buildPotentialCentralityThresholds(transformedSnapshot.potentialPeers);
  const openingResults = (Array.isArray(transformedSnapshot.potentialPeers)
    ? transformedSnapshot.potentialPeers
    : [])
    .map((peer) =>
      buildChannelOpeningRecommendation(
        peer,
  ...
  const feeRecommendations = feeResults.map((item) => item.recommendation);
  const channelOpeningRecommendations = openingResults.map((item) => item.recommendation);
  ...
  return {
    ...
    recommendationSet: {
      ...
      summary: {
        ...
        openingCount: channelOpeningRecommendations.length,
      },
    },
    ...(debugEnabled
      ? {
          debugTrace: {
            schemaVersion: "prototype-debug-trace-v1",
            generatedAt: new Date().toISOString(),
            telemetrySummary: {
              nodeAlias: String(telemetry?.nodeAlias || "prototype-node"),
              channelCount: Array.isArray(telemetry?.channels) ? telemetry.channels.length : 0,
              potentialPeerCount: Array.isArray(telemetry?.potentialPeers) ? telemetry.potentialPeers.length : 0,
            },
            feeRecommendationEvaluations: feeResults.map((item) => item.debug),
            openingCandidateEvaluations: openingResults.map((item) => item.debug),
          },
        }
      : {}),
  };
...
const buildSignerKeyPolicy = (runtime, extra = {}) => ({
  requireExplicitKey: runtime.requireExplicitKey === true,
  keySource: runtime.keySource,
  signerProviderId: runtime.providerId,
  signerProviderType: runtime.providerType,
  signingMode: runtime.signingMode,
  ...(runtime.keyProviderId ? { keyProviderId: runtime.keyProviderId } : {}),
  ...(runtime.releasedSignerPolicy ? { releasedSignerPolicy: runtime.releasedSignerPolicy } : {}),
  ...extra,
});
...
const createSignerProviderRuntime = () => {
      ...
      getReadiness() {
        if (
          ...
          return {
            ...
            keyPolicy: buildSignerKeyPolicy(this, { requireExplicitKey: true }),
          };
        }

        return {
          ok: true,
          keyPolicy: buildSignerKeyPolicy(this),
        };
      },
      signDigest(digest) {
        return {
  ...
  if (PROTOTYPE_SIGNER_PROVIDER_TYPE === "released_keyring") {
    const releasedSignerPolicy = buildReleasedSignerPolicy();
    let keyring = {};
    const configErrors = [];

    if (!PROTOTYPE_RELEASED_SIGNER_KEY_ID) {
      configErrors.push("PROTOTYPE_RELEASED_SIGNER_KEY_ID is required for signerProviderType released_keyring.");
    }
    if (!PROTOTYPE_RELEASED_SIGNER_KEYRING_JSON) {
      configErrors.push("PROTOTYPE_RELEASED_SIGNER_KEYRING_JSON is required for signerProviderType released_keyring.");
    }
    if (!PROTOTYPE_ATTESTATION_INCLUDE) {
      configErrors.push("released_keyring signer provider requires PROTOTYPE_ATTESTATION_INCLUDE=true.");
    }

    if (PROTOTYPE_RELEASED_SIGNER_KEYRING_JSON) {
      try {
        keyring = parseJsonObjectEnv(PROTOTYPE_RELEASED_SIGNER_KEYRING_JSON, "PROTOTYPE_RELEASED_SIGNER_KEYRING_JSON");
      } catch (error) {
    ...
    if (
      ...
      !Object.prototype.hasOwnProperty.call(keyring, PROTOTYPE_RELEASED_SIGNER_KEY_ID)
    ) {
      configErrors.push(
        `PROTOTYPE_RELEASED_SIGNER_KEYRING_JSON is missing keyId ${PROTOTYPE_RELEASED_SIGNER_KEY_ID}.`
      );
    }

    return {
      providerId: PROTOTYPE_SIGNER_PROVIDER_ID,
      providerType: "released_keyring",
      algorithm: "hmac-sha256",
      keyId: PROTOTYPE_RELEASED_SIGNER_KEY_ID,
      keySource: PROTOTYPE_RELEASED_SIGNER_KEY_PROVIDER_ID,
      keyProviderId: PROTOTYPE_RELEASED_SIGNER_KEY_PROVIDER_ID,
      signingMode: "released_signer",
      requireExplicitKey: true,
      requiresAttestation: true,
      releasedSignerPolicy,
      getReadiness() {
      ...
      releaseKey(attestation) {
        ...
        const decision = evaluateReleasedSignerPolicy(attestation, releasedSignerPolicy);
        if (!decision.ok || !decision.keyId) {
          return {
            ok: false,
        ...
        const keyMaterial = keyring[decision.keyId];
        if (!keyMaterial) {
          return {
            ok: false,
            errors: [`Released signer keyId ${decision.keyId} was not found in the configured keyring.`],
            warnings: decision.warnings,
            keyId: null,
            keyMaterial: null,
            keySource: this.keySource,
      ...
      signDigest(digest, options = {}) {
        const release = this.releaseKey(options.attestation || null);
        if (!release.ok || !release.keyMaterial || !release.keyId) {
          throw new Error(`Released signer key release denied: ${release.errors.join(" | ")}`);
        }

        return {
          algorithm: this.algorithm,
          keyId: release.keyId,
          keySource: release.keySource,
          signerProviderId: this.providerId,
          signerProviderType: this.providerType,
          value: hmacSha256Hex(digest, release.keyMaterial),
        };
      },
      verifyDigest(digest, signature) {
        const errors = [];
        const signatureKeyId = typeof signature?.keyId === "string" ? signature.keyId : this.keyId;
        const keyMaterial = signatureKeyId ? keyring[signatureKeyId] : null;
        const expectedSignature = keyMaterial ? hmacSha256Hex(digest, keyMaterial) : null;
        ...
        if (!expectedSignature) {
          errors.push(`signature.keyId not found in released signer keyring (${signatureKeyId || "missing"}).`);
        } else if (signature?.value !== expectedSignature) {
...
const buildSignerProviderStatus = () => ({
  providerId: SIGNER_PROVIDER.providerId,
  providerType: SIGNER_PROVIDER.providerType,
  signingMode: SIGNER_PROVIDER.signingMode,
  keySource: SIGNER_PROVIDER.keySource,
  ...(SIGNER_PROVIDER.keyProviderId ? { keyProviderId: SIGNER_PROVIDER.keyProviderId } : {}),
  ...(SIGNER_PROVIDER.releasedSignerPolicy
    ? { releasedSignerPolicy: SIGNER_PROVIDER.releasedSignerPolicy }
    : {}),
});
...
const buildPrototypeArb = async (scored, sourceReceipt) => {
  ...
  const attestation = PROTOTYPE_ATTESTATION_INCLUDE || SIGNER_PROVIDER.requiresAttestation
    ? (await resolvePrototypeRuntimeAttestationEvidence(digest)).arbAttestation
    : null;
  const signature = SIGNER_PROVIDER.signDigest(digest, { attestation });
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && url.pathname === "/api/recommend") {
    ...
    const debugTraceEnabled = wantsDebugTrace(url);
    const scored = scorePrototypeRecommendations(telemetry, { debug: debugTraceEnabled });
    ...
    try {
      ...
      const payload = {
        ...
        privacyMode: "feature_only",
        modelVersion: scored.modelVersion,
        signingMode: SIGNER_PROVIDER.signingMode || "prototype_hmac",
        transformedSnapshot: scored.transformedSnapshot,
        recommendationSet: scored.recommendationSet,
        sourceReceipt,
        arb,
        ...(debugTraceEnabled ? { debugTrace: scored.debugTrace } : {}),
      };
      if (debugTraceEnabled) {
        console.info(
          `[${SERVICE_NAME}] debugTrace`,
          JSON.stringify(
            {
              telemetryHash: sha256Hex(telemetry || {}),
              transformedSnapshotHash: sha256Hex(scored.transformedSnapshot || {}),
              recommendationSetHash: sha256Hex(scored.recommendationSet || {}),
              debugTrace: scored.debugTrace,
            },
            null,
            2
          )
