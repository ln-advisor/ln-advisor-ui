...
const scorePrototypeRecommendations = (telemetry) => {
  ...
  const channelOpeningRecommendations = (Array.isArray(transformedSnapshot.potentialPeers)
    ? transformedSnapshot.potentialPeers
    : [])
    .map((peer) =>
      buildChannelOpeningRecommendation(
        peer,
        buildPotentialCentralityThresholds(transformedSnapshot.potentialPeers)
      )
    )
