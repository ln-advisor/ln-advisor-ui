...
const logOpeningDebug = (label, payload) => {
  ...
  console.info(`[Opening Recs] ${label}`, payload);
};
...
const buildOutgoingInspector = ({ mode, propsPayload, standardResponse = null, phalaResponse = null }) => {
  if (mode === 'phala_verified') {
    ...
    return { route: 'Verified Phala', transport: import.meta.env.DEV ? 'Browser -> Vite proxy -> Phala' : 'Browser -> Phala', requests };
  }
...
const RecommendationsPage = ({ lnc, darkMode, mockSnapshot = null }) => {
  ...
  const executeReviewedPhalaRun = useCallback(async (preparedRun) => {
    ...
    try {
      ...
      if (items.length === 0) throw new Error('Phala verified path returned no opening recommendations.');
      ...
      console.error('[Opening Recs] advisor failed', err);
  ...
  const runAdvisor = useCallback(async () => {
    ...
    try {
      ...
      if (activeAnalysisMode === 'phala_verified') {
        ...
        if (items.length === 0) throw new Error('Props API returned no opening recommendations.');
    ...
    <div className="px-6 pb-12 pt-8 space-y-8" style={{ maxWidth: 1200, margin: '0 auto' }}>
      <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="mt-1 flex items-center gap-2">
                <SectionBadge label="Props" variant="public" />
          ...
          <p className="max-w-xl text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>Generate candidate peers from graph centrality, current channels, mission control signals, and a privacy-reduced payload.</p>
        </div>
        <div className="flex flex-col gap-3 md:items-end">
          {phalaModeEnabled && (
            <div className="flex items-center gap-1 rounded-lg border p-1" style={{ borderColor: darkMode ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)', backgroundColor: darkMode ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)' }}>
              ...
              <button type="button" onClick={() => phalaModeAvailable && setAnalysisMode('phala_verified')} disabled={!phalaModeAvailable} className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-widest ${!phalaModeAvailable ? 'opacity-40 cursor-not-allowed' : ''}`} style={{ backgroundColor: activeAnalysisMode === 'phala_verified' ? 'rgba(59,130,246,0.18)' : 'transparent', color: activeAnalysisMode === 'phala_verified' ? '#60a5fa' : 'var(--text-secondary)' }}>Verified Phala</button>
            </div>
          )}
      ...
      {(recommendations.length > 0 || verifyResult || phalaRun) && (
        <div className="rounded-2xl p-5 space-y-4" style={{ backgroundColor: 'var(--bg-card)', border: `1px solid ${darkMode ? 'rgba(255,255,255,0.08)' : 'rgba(15,23,42,0.08)'}` }}>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3 text-sm">
            <div className="flex items-center justify-between"><span style={{ color: 'var(--text-secondary)' }}>Analysis Route</span><span className="font-mono text-xs" style={{ color: activeAnalysisMode === 'phala_verified' ? '#60a5fa' : 'var(--text-primary)' }}>{activeAnalysisMode === 'phala_verified' ? 'Verified Phala' : 'Standard API'}</span></div>
            <div className="flex items-center justify-between"><span style={{ color: 'var(--text-secondary)' }}>Verification</span><span className="font-mono text-xs" style={{ color: verifyResult?.ok ? '#22c55e' : '#f97316' }}>{verifyResult ? (verifyResult.ok ? 'ARB verified' : 'Verification failed') : 'Pending'}</span></div>
          ...
          {phalaRun && (
            <div className="rounded-xl p-4" style={{ backgroundColor: darkMode ? 'rgba(59,130,246,0.08)' : 'rgba(59,130,246,0.06)', border: `1px solid ${darkMode ? 'rgba(96,165,250,0.18)' : 'rgba(59,130,246,0.14)'}` }}>
              <div className="mb-3 text-[10px] font-bold uppercase tracking-widest text-blue-400">Phala Trust Status</div>
              <div className="grid grid-cols-1 gap-2 md:grid-cols-2 text-[11px]">
                ...
                <div className="flex items-center justify-between"><span style={{ color: 'var(--text-secondary)' }}>Quote Check</span><span className="font-mono" style={{ color: phalaRun?.verify?.cloudVerification?.quoteVerified ? '#22c55e' : '#f97316' }}>{phalaRun?.verify?.cloudVerification?.quoteVerified ? 'Cloud verified' : 'Unavailable'}</span></div>
                ...
                <div className="flex items-center justify-between"><span style={{ color: 'var(--text-secondary)' }}>Attestation Source</span><span className="font-mono" style={{ color: 'var(--text-primary)' }}>{phalaRun?.health?.attestationSource || '-'}</span></div>
                <div className="flex items-center justify-between"><span style={{ color: 'var(--text-secondary)' }}>Live Evidence</span><span className="font-mono" style={{ color: verifyResult?.liveAppEvidencePolicy?.requireLiveAppEvidence ? '#22c55e' : 'var(--text-secondary)' }}>{verifyResult?.liveAppEvidencePolicy?.requireLiveAppEvidence ? 'Required' : 'Optional'}</span></div>
              </div>
            </div>
          )}
        </div>
